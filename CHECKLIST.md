...
- [ ] **Создание объекта**
  - Кнопка "➕ Добавить объект" показывает выбор режима
  - Импорт PDF корректно заполняет поля и позволяет вносить правки текстом/голосом
  - Ручной режим (12 шагов) проходит успешно
  - Объект создается в БД и появляется в "Текущие объекты"
...
- [ ] **Добавление расходов**
  - Кнопка "➕ Добавить расход" → выбор типа работает
  - Текстовый ввод парсится через AI
  - Данные корректные (дата, сумма, описание)
  - Расход сохраняется, чек прикрепляется (если есть)

- [ ] **Голосовой ввод**
  - Отправлено голосовое сообщение
  - Голос распознан через Whisper
  - Данные извлечены корректно / правка по голосу применяется
- [ ] **Просмотр авансов**
  - Кнопка "📄 Посмотреть авансы" отображает историю и суммы
