...
async def parse_pdf_to_object_data(text: str) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "Облицовка С3 (estimate_s3 и actual_s3_discount) включает только материалы с формулировками: "
        "'облицовочные элементы', 'затирка эпоксидная С3', 'покрытие финишное', любые упоминания С3 в связке с облицовкой. "
        "Другие строки (например, 'расходные материалы', вспомогательные смеси, крепёж и т.п.) относятся к расходникам (estimate_supplies). "
        "Не суммируй все материалы в С3, разделяй строго по этому правилу."
    )

    user_prompt = (
        "Ниже текст сметы. Извлеки данные и верни JSON со строгой схемой.\n"
        "Материалы С3: учитывай только строки, связанные с облицовочными элементами, затиркой С3 'Зубр', покрытием финишным 'Тритон' и аналогами.\n"
        "Все остальные материалы относись к расходникам. Если данных нет — null. Суммы возвращай числом (можно с точкой).\n\n"
        "Текст сметы:\n"
        f"{text}"
    )
